...
function calculateLeaderboardPlaces(users, minScores) {
  ...
  const thresholds = [
    { minScore: firstPlaceMinScore, place: 1 },
    { minScore: secondPlaceMinScore, place: 2 },
    { minScore: thirdPlaceMinScore, place: 3 },
  ];
  ...
  for (const user of sortedUsers) {
    const thresholdMatch = thresholds.find(({ minScore }) => user.score >= minScore);

    const isSameAsPrevious = user.score === prevScore;

    const place = thresholdMatch
      ? thresholdMatch.place
      : isSameAsPrevious
        ? prevPlace
        : nextPlace++;

    result.push({ userId: user.userId, place });
